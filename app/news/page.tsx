...
export default async function NewsList() {
  ...
  const restNews = news.length > 1 ? news.slice(1) : [];
    ...
    <div className="overflow-x-hidden pb-28">
      ...
      <div className="max-w-7xl mx-auto px-6">
        {featured ? (
                ...
                <Link href={`/news/${featured.id}`} className="group block">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center bg-white rounded-2xl md:rounded-[32px] overflow-hidden shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-gray-100 transition-all duration-300 hover:shadow-xl">
                    <div className="relative aspect-[4/3] lg:aspect-auto lg:h-[400px] w-full bg-gray-100 overflow-hidden">
                      <Image fill src={featured.image || "/heroHome.png"} alt={featured.title} className="object-cover group-hover:scale-105 transition-transform duration-500" />
                    </div>
                    <div className="p-8 lg:p-12 lg:pr-16">
                      <div className="flex items-center gap-3 mb-4 text-sm font-semibold">
                        ...
                        <span className="text-gray-400">{formatDate(featured.created_at)}</span>
                      </div>
                ...
                <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {restNews.map((newsItem) => (
                    <StaggerItem key={newsItem.id}>
                      <Link href={`/news/${newsItem.id}`} className="group block h-full">
                        <div className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all border border-gray-100 h-full flex flex-col hover:-translate-y-1 duration-300">
                          <div className="relative aspect-[16/9] w-full bg-gray-100 overflow-hidden">
                            <Image fill src={newsItem.image || "/heroHome.png"} alt={newsItem.title} className="object-cover group-hover:scale-105 transition-transform duration-500" />
                          </div>
                          <div className="p-6 flex flex-col flex-1">
                            <div className="flex items-center gap-2 mb-3 text-xs font-semibold">
                              ...
                              <span className="text-gray-500">{formatDate(newsItem.created_at)}</span>
                            </div>
          ...
          <div className="text-center text-gray-500 py-12">Belum ada berita yang diterbitkan.</div>
        )}
      </div>
    </div>
  );
